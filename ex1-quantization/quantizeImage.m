...
% quantizeImage quantization of an image to nQuants.
  try
        ...
        if nQuant < sum(histOrig > 0)
            ...
            if flag_RGB
                ...
                imOrig_YIQ(:,:,1) = final_result;
                imQuant = transformYIQ2RGB(imOrig_YIQ);
            else
                imQuant = final_result;
            end
        end
        
        % Show the quantized image alongside the original image
        figure;
        imshow([imOrig imQuant]);
